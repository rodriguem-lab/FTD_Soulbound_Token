...
contract FTDSoulbound is ERC721, Ownable {
    ...
    // Mint en batch (cohorte) optimisé
    ...
    // Fonction interne pour factoriser le mint
    ...
    // RÉVOCATION CORRIGÉE : Libère le statut hasMinted
    function revoke(uint256 tokenId) external onlyOwner {
        address student = ownerOf(tokenId); // On récupère l'étudiant avant de brûler le token
        
        hasMinted[student] = false; // L'étudiant peut à nouveau recevoir un token
        _burn(tokenId);
    ...
    // Logique Soulbound (non-transférable)
